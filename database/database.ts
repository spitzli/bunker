...
import * as custom from "../modules/database/mod.ts";
...
export const db = {
  ...
  ...mammoth.defineDb(custom, async (query: string, params: any[]) => {
    logger.debug("[MODULE DATABASE] running query", query, params);

    const dbResult = await sql.unsafe(query, params);

    logger.debug("[MODULE DATABASE] query result", query, params, dbResult);
