...
async function validSession(
  ...
  const valid = await db
    ...
    .from(db.sessions)
    .where(
      db.sessions.sessionId.eq(sessionId).and(db.sessions.userId.eq(userId))
    )
...
export async function getJwtPayload<T extends {}>(
  token: string
): Promise<T | undefined> {
  return (await jwt
    .verify(token, publicKey, "RS512")
...
export async function jwtMiddleware(
  ...
  if (refreshToken) {
    ...
    if (!payload) {
      ...
      if (decoded) {
        db.deleteFrom(db.sessions).where(
          db.sessions.sessionId.eq(decoded.sessionId)
        );
    ...
    const user: User = {
      ...(await getUser(BigInt(payload.id))),
